...
from PySide6.QtCore import Qt, Signal, QObject, QTimer, QUrl, QMimeData, QSize, QRectF, QEvent, QStandardPaths, QPoint
from PySide6.QtGui import QPixmap, QImage, QIcon, QDrag, QAction, QPainter, QPen, QColor, QLinearGradient, QPainterPath
...
class SettingsDialog(QDialog):
    ...
    def export_diagnostics(self):
        ...
        for c in self.cards:
            p = POLICIES[c.device_id]; data["displays"].append({"name": c.title_text, "vid_pid": c.device_id, "connection": c.connection.text(), "transmitted_resolution": list(c.size_target), "mapping": c.mapping, "requested_fps": c.fps.currentText(), "actual_fps": c.actual_fps(), "usb_bytes_per_second": c.session.metrics.usb_bytes_per_second, "last_send_ms": c.session.metrics.last_send_ms, "ack_latency_ms": c.session.metrics.ack_latency_ms, "dropped_frames": c.drop_count(), "last_error": c.session.metrics.last_error, "persistence_confidence": p.confidence})
        Path(path).write_text(json.dumps(data, indent=2), encoding="utf-8")

class HardwareMonitorDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent); self.setWindowTitle("Hardware Monitor Designer"); self.resize(900, 520); v = QVBoxLayout(self); row = QHBoxLayout(); self.target = QComboBox(); self.target.addItem('9.16" LCD', "0416:5408"); self.target.addItem('6" LCD', "0416:5302"); self.template = QComboBox(); self.template.addItems(["Compact", "Gaming", "Performance", "Temperatures", "Network", "Minimal"]); row.addWidget(QLabel("Display")); row.addWidget(self.target); row.addWidget(QLabel("Template")); row.addWidget(self.template); row.addStretch(); v.addLayout(row)
    ...
    def refresh(self):
        target = self.target.currentData(); layout = templates(target)[self.template.currentText()]; sample = {"game.fps": 144, "game.frametime": 6.9, "gpu.usage": 82, "gpu.temperature": 63, "gpu.hotspot": 72, "cpu.temperature": 55, "cpu.usage": 34, "memory.usage": 42, "network.download": 240, "network.upload": 32}
...
class DisplayCard(QFrame):
    ...
    def __init__(self, title, size, device_id, parent=None, hardware_sender=None, media_directory_provider=None, media_directory_selected=None):
        ...
        self._last_delivered_frame_ref = None
        ...
        for btn, width in ((self.footer_change, 58), (self.footer_clear, 48), (self.footer_play, 48), (self.footer_pause, 54), (self.footer_stop, 48)):
        ...
        self.footer_x_label = QLabel("X"); self.footer_x_label.setFixedWidth(10); footer.addWidget(self.footer_x_label); footer.addWidget(self.pan_x_box); footer.addSpacing(2)
        self.footer_y_label = QLabel("Y"); self.footer_y_label.setFixedWidth(10); footer.addWidget(self.footer_y_label); footer.addWidget(self.pan_y_box); footer.addSpacing(2)
        self.footer_zoom_label = QLabel("Zoom"); self.footer_zoom_label.setFixedWidth(34); footer.addWidget(self.footer_zoom_label); footer.addWidget(self.zoom_box); footer.addSpacing(2)
        self.footer_rotation_label = QLabel("Rotation"); self.footer_rotation_label.setFixedWidth(50); footer.addWidget(self.footer_rotation_label); footer.addWidget(self.rotation); footer.addSpacing(2)
        ...
        self.unified_inspector = QFrame(); self.unified_inspector.setObjectName("unifiedInspector"); self.unified_inspector.setFixedWidth(300)
        ...
        self.choose_button = QPushButton("Change"); self.choose_button.setFixedWidth(64); self.choose_button.setFixedHeight(26); self.choose_button.clicked.connect(self.choose)
        ...
        # Compact numeric transform controls used only in Side-by-Side.
        # They mirror the existing footer controls; no transform/backend logic changes.
        self.sbs_transform_widget = QWidget()
        sbs_t = QGridLayout(self.sbs_transform_widget); sbs_t.setContentsMargins(0, 2, 0, 0); sbs_t.setHorizontalSpacing(4); sbs_t.setVerticalSpacing(3)

        self.sbs_x = QSpinBox(); self.sbs_y = QSpinBox(); self.sbs_zoom = QDoubleSpinBox(); self.sbs_rotation = QComboBox()
        for box in (self.sbs_x, self.sbs_y):
            box.setRange(-8192, 8192); box.setButtonSymbols(QSpinBox.NoButtons); box.setFixedWidth(54); box.setFixedHeight(24)
        self.sbs_zoom.setRange(1, 8); self.sbs_zoom.setDecimals(2); self.sbs_zoom.setSingleStep(.05); self.sbs_zoom.setValue(1); self.sbs_zoom.setButtonSymbols(QDoubleSpinBox.NoButtons); self.sbs_zoom.setFixedWidth(58); self.sbs_zoom.setFixedHeight(24)
        self.sbs_rotation.addItems(["0°", "90°", "180°", "270°"]); self.sbs_rotation.setFixedWidth(62); self.sbs_rotation.setFixedHeight(24)

        sbs_t.addWidget(QLabel("X"), 0, 0); sbs_t.addWidget(self.sbs_x, 0, 1)
        sbs_t.addWidget(QLabel("Y"), 0, 2); sbs_t.addWidget(self.sbs_y, 0, 3)
        sbs_t.addWidget(QLabel("Zoom"), 1, 0); sbs_t.addWidget(self.sbs_zoom, 1, 1)
        sbs_t.addWidget(QLabel("Rot"), 1, 2); sbs_t.addWidget(self.sbs_rotation, 1, 3)

        self.sbs_transform_widget.hide()
        trans_grid.addWidget(self.sbs_transform_widget, 1, 0, 1, 4)
        ...
        self.output_selector = QComboBox(); self.output_selector.hide()
        self.output_segments = {}
        out_row = QHBoxLayout(); out_row.setSpacing(4)
        for label, val in (("Media", OutputMode.MEDIA.value), ("Sensor Theme", OutputMode.HARDWARE_MONITOR.value), ("OFF", OutputMode.STOPPED.value)):
            btn = QPushButton(label); btn.setCheckable(True); btn.setFixedHeight(26)
        ...
        self.mode = QComboBox(); self.mode.addItems([x.value for x in FitMode]); self.mode.setMaximumWidth(105); self.mode.hide()
        self.quality_box = QComboBox(); self.quality_box.addItems(QUALITY_PROFILES); self.quality_box.hide()
        self.auto_preview = QCheckBox(); self.auto_preview.setChecked(True); self.auto_preview.hide()
        self.preview_scale_slider = QSlider(Qt.Horizontal); self.preview_scale_slider.setValue(100); self.preview_scale_slider.hide()
        self.preview_scale_value = QLabel("Auto"); self.preview_scale_value.hide()
        self.context_tabs = QTabWidget(); self.context_tabs.hide()
        self.profile_box = QComboBox(); self.profile_box.setEditable(True); self.profile_box.setInsertPolicy(QComboBox.NoInsert); self.profile_box.setMaximumWidth(170); self.profile_box.setMinimumWidth(120); self.profile_box.hide()
        self.advanced_toggle = QToolButton(); self.advanced_toggle.hide()
        self.advanced = QWidget(); self.advanced.hide()
        self.status = QLabel("READY"); self.status.hide()
        self.output_metrics = QLabel(""); self.output_metrics.hide()
        self.metrics = QLabel(""); self.metrics.hide()
        self.connection_retry = QPushButton(); self.connection_retry.hide()
        self.connection_reconnect = QPushButton(); self.connection_reconnect.hide()
        ...
        self.pan_x_box.valueChanged.connect(lambda v: (self.sbs_x.setValue(v), self._transform_changed()))
        self.pan_y_box.valueChanged.connect(lambda v: (self.sbs_y.setValue(v), self._transform_changed()))
        self.zoom_box.valueChanged.connect(lambda v: (self.sbs_zoom.setValue(v), self._transform_changed()))
        self.rotation.currentIndexChanged.connect(lambda i: (self.sbs_rotation.setCurrentIndex(i), self._transform_changed()))
        self.sbs_x.valueChanged.connect(self.pan_x_box.setValue)
        self.sbs_y.valueChanged.connect(self.pan_y_box.setValue)
        self.sbs_zoom.valueChanged.connect(self.zoom_box.setValue)
        self.sbs_rotation.currentIndexChanged.connect(self.rotation.setCurrentIndex)
        ...
        self.play_button = QPushButton("Show"); self.pause_button = QPushButton("Pause"); self.stop_button = QPushButton("Stop"); self.clear_button = QPushButton("Clear")
        ...
        self.reset_transform_button = QPushButton("Reset"); self.reset_transform_button.hide(); self.reset_transform_button.clicked.connect(self.reset_transform)
        self.quality_label = QLabel("Quality"); self.quality_label.hide()
        self.quality_box = QComboBox(); self.quality_box.addItems(QUALITY_PROFILES); self.quality_box.setCurrentText("Balanced"); self.quality_box.hide(); self.quality_box.currentTextChanged.connect(self._quality_changed)
        self.theme_label = QLabel("Theme"); self.theme_label.hide()
        self.inspector_thumbnail = QLabel(); self.inspector_thumbnail.setFixedSize(38,25); self.inspector_thumbnail.hide()
        ...
        self.media_tab = QWidget(); self.media_tab_layout = QVBoxLayout(self.media_tab)
        self.transform_tab = QWidget(); self.transform_tab_layout = QVBoxLayout(self.transform_tab)
        self.monitor_tab = QWidget(); self.monitor_tab_layout = QVBoxLayout(self.monitor_tab)
        self.device_tab = QWidget(); self.device_tab_layout = QVBoxLayout(self.device_tab)
        ...
        self.profile_save_button = QPushButton("Save"); self.profile_save_button.setFixedWidth(68); self.profile_save_button.hide()
        self.profile_save_as_button = QPushButton("Save As"); self.profile_save_as_button.setFixedWidth(78); self.profile_save_as_button.hide()
        self.profile_manage_button = QToolButton(); self.profile_manage_button.setText("⋯"); self.profile_manage_button.hide()
        ...
        self.monitor_save_button = QPushButton("Save"); self.monitor_save_as_button = QPushButton("Save As…"); self.monitor_reset_button = QPushButton("Reset preset"); self.monitor_edit_hint = QLabel("Turn on Edit Layout, then click an element in the preview.")
        ...
        for _label,_value in (("Media",OutputMode.MEDIA),("Sensor Theme",OutputMode.HARDWARE_MONITOR),("Off",OutputMode.STOPPED)):
    ...
    def _fit_side_by_side_preview(self):
        ...
        available_w = max(320, self.preview_column.contentsRect().width() - 10)
        ratio = 4.0 if self.is_wide else (1280 / 480)

        # Keep a small breathing margin but otherwise make the LCD preview as
        # large as the half-screen card allows.
        width = max(320, int(available_w * 0.96))
        height = max(100, int(round(width / ratio)))

        # There is plenty of vertical room in Side-by-Side.  These caps prevent
        # the preview from colliding with the footer on unusually small windows.
        max_h = 220 if self.is_wide else 285
        if height > max_h:
            height = max_h
    ...
    def set_layout_mode(self, side_by_side: bool):
        ...
        if side_by_side:
            # The footer no longer carries duplicated transform controls, so the
            # inspector can be narrower and the preview can use the freed width.
            inspector_width = 190
        else:
            inspector_width = 300

        self.unified_inspector.setFixedWidth(inspector_width)
        ...
        # The old Side-by-Side clipping came from the footer row: all of the
        # fixed-width media + Fit/Fill/Center + X/Y/Zoom/Rotation widgets made
        # each card demand ~900 px.  In Side-by-Side, the duplicated transform
        # controls move into the inspector instead, shrinking the card minimum
        # width while keeping every function available.
        for button in self.footer_mode_buttons.values():
            button.setVisible(not side_by_side)

        for widget in (
            self.footer_x_label, self.pan_x_box,
            self.footer_y_label, self.pan_y_box,
            self.footer_zoom_label, self.zoom_box,
            self.footer_rotation_label, self.rotation,
            self.footer_reset,
        ):
            widget.setVisible(not side_by_side)

        self.sbs_transform_widget.setVisible(side_by_side)

        # Shorter output label only while compact; full wording returns in Stacked.
        sensor_button = self.output_segments.get(OutputMode.HARDWARE_MONITOR.value)
        if sensor_button is not None:
            sensor_button.setText("Sensor" if side_by_side else "Sensor Theme")
    ...
    def update_output_controls(self):
        theme=self.output_selector.currentData()==OutputMode.HARDWARE_MONITOR.value
    ...
    def _update_media_controls(self):
        ...
        self.play_button.setText("Show" if photo else "Play");self.choose_button.setText("Change")
    ...
    def _brightness_changed(self,value):self.brightness_value.setText(f"{value}%");self.header_brightness.setText(f"Brightness {value}%");self._transform_changed()
    ...
    def refresh(self):
        ...
        try:
            ...
            if not self.hardware_started:
                ...
                self._show_image(self._qimage(prepared.canvas,720,190));prepared.canvas.close()
            ...
            self._show_image(self._qimage(prepared.canvas,720,190))
    ...
    def _consume_preview(self):
        ...
        if image is not None and self.preview_enabled:self._show_image(image)
    ...
    def _update_preview_geometry(self):
        ...
        base_w=getattr(self,"_preview_base_width",720 if self.device_id.endswith("5408") else 600)
        scale=getattr(self,"preview_scale",100)/100
        width=max(280,int(base_w*scale));height=max(105,int(round(width/ratio)))
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("Oni Thermal LCD Control"); self.resize(1600, 960); self.setMinimumSize(820, 560)
        ...
        self.sidebar = QFrame(); self.sidebar.setObjectName("sidebar"); self.sidebar.setFixedWidth(180)
        side = QVBoxLayout(self.sidebar); side.setContentsMargins(8, 16, 8, 12); side.setSpacing(6)
        ...
        self.brand_icon = QLabel(); self.brand_icon.hide()
        self.brand_lbl = QLabel("<span style='font-size:20pt;font-weight:800;letter-spacing:1px;color:#ffffff'>ONI</span>")
        ...
        self.nav = {}
        nav_items = [
            ("Home", "⌂"), ("Media Library", "▣"), ("Sensor Themes", "✦"),
            ("Profiles", "◇"), ("Hardware Monitor", "⌁"), ("Performance", "↗"),
            ("Settings", "⚙"), ("Diagnostics", "⋯")
        ]
        self.pages = QStackedWidget()
        for i, (name, icon) in enumerate(nav_items):
            btn = QPushButton(f"{icon} {name}"); btn.setObjectName("navButton"); btn.setCheckable(True)
            btn.setProperty("fullText", name); btn.setProperty("compactText", icon); btn.setToolTip(name)
        ...
        self.layout_selector = QComboBox(); self.layout_selector.addItem("Stacked", "stacked"); self.layout_selector.addItem("Side by Side", "side_by_side"); self.layout_selector.setMaximumWidth(126); self.layout_selector.hide()
        self.layout_buttons = {}
        ...
        self.left = DisplayCard('THERMALRIGHT · Trofeo Vision 9.16 LCD', (1920, 462), "0416:5408", media_directory_provider=chooser_provider, media_directory_selected=chooser_selected)
        self.right = DisplayCard('THERMALRIGHT · Trofeo Vision LCD', (1280, 480), "0416:5302", media_directory_provider=chooser_provider, media_directory_selected=chooser_selected)
        self.card_by_id = {self.left.device_id: self.left, self.right.device_id: self.right}

        self.global_brightness.valueChanged.connect(lambda val: (self.global_brightness_val.setText(f"{val}%"), self.left.brightness_slider.setValue(val), self.right.brightness_slider.setValue(val)))

        self.display_splitter = QSplitter(Qt.Vertical)
        self.display_splitter.setChildrenCollapsible(False)
        self.display_splitter.addWidget(self.left); self.display_splitter.addWidget(self.right)
        ...
        self.pages.addWidget(self._action_page(
            "Media Library",
            "Organize reusable images, GIFs and videos. The library keeps media references available for both LCDs.",
            "Open Media Library",
            self.open_library
        ))
        self.pages.addWidget(self._action_page(
            "Sensor Themes",
            "Browse and edit SensorPanel-style themes, backgrounds, text, gauges, graphs and live sensor bindings.",
            "Open Sensor Theme Gallery",
            self.open_theme_gallery
        ))
        self.pages.addWidget(self._action_page(
            "Profiles",
            "Create, rename, duplicate, import and export independent display profiles.",
            "Manage Profiles",
            self.open_profiles
        ))
        self.pages.addWidget(self._hardware_monitor_page())
        self.pages.addWidget(self._performance_page())
        self.pages.addWidget(self._settings_page())
        self.pages.addWidget(self._diagnostics_page())
        shell.addWidget(self.pages, 1)

        self.sync_controller = DisplaySyncController((self.left, self.right))
        self.left.coordinator = self.sync_controller; self.right.coordinator = self.sync_controller
        ...
        self.sync_controller=DisplaySyncController((self.left,self.right),prepare_shared=self._prepare_shared_decode,stop_shared=self._stop_shared_decode)
        self.left.coordinator=self.sync_controller;self.right.coordinator=self.sync_controller
        self.sync_toggle.toggled.connect(self._sync_changed)
        self.left.brightness_slider.valueChanged.connect(lambda value:self._sync_brightness(self.left,value)); self.right.brightness_slider.valueChanged.connect(lambda value:self._sync_brightness(self.right,value))
        self.left.mediaChanged.connect(lambda _:self._stop_shared_decode()); self.right.mediaChanged.connect(lambda _:self._stop_shared_decode())
        ...
        self.settings.display_order=[x for x in getattr(self.settings,"display_order",[]) if x in self.card_by_id] or ["0416:5408","0416:5302"]
        ...
        if "0416:5408" in _profile:self.left.apply_profile(_profile["0416:5408"])
        if "0416:5302" in _profile:self.right.apply_profile(_profile["0416:5302"])
        self.left.profileSelected.connect(lambda name:self.apply_device_profile(name,"0416:5408",self.left));self.right.profileSelected.connect(lambda name:self.apply_device_profile(name,"0416:5302",self.right))
        self.left.profileSaveRequested.connect(lambda name:self.save_device_profile(name,"0416:5408",self.left));self.right.profileSaveRequested.connect(lambda name:self.save_device_profile(name,"0416:5302",self.right))
        self.left.profileSaveAsRequested.connect(lambda name:self.save_device_profile(name,"0416:5408",self.left,save_as=True));self.right.profileSaveAsRequested.connect(lambda name:self.save_device_profile(name,"0416:5302",self.right,save_as=True))
        self.left.profileManageRequested.connect(self.open_profiles);self.right.profileManageRequested.connect(self.open_profiles)
        ...
        if self.settings.window_maximized:QTimer.singleShot(0,self.showMaximized)
    ...
    def set_sidebar_expanded(self, expanded, save=True):
        ...
        self.sidebar.setFixedWidth(140 if expanded else 58)
        self.brand_lbl.setVisible(expanded)
        for button in self.nav.values():
            button.setText((f"{button.property('compactText')} {button.property('fullText')}") if expanded else str(button.property("compactText")))
        ...
        if hasattr(self,"left"):
            QTimer.singleShot(0,self.left._update_preview_geometry);QTimer.singleShot(0,self.right._update_preview_geometry)
    ...
    def select_page(self, index, open_dialog=True):
        ...
        # These sections are dialog-backed in the existing application.  Open
        # them immediately from the sidebar while still leaving a useful page
        # behind instead of the old blank QWidget.
        if not open_dialog: return
        callbacks = {
            1: self.open_library,
            2: self.open_theme_gallery,
            3: self.open_profiles,
            4: self.open_monitor_designer,
    ...
    def _diagnostics_page(self):
        ...
        refresh.clicked.connect(self.refresh_runtime_diagnostics); sensors.clicked.connect(self.refresh_sensor_page); export.clicked.connect(lambda: SettingsDialog(self.settings, (self.left, self.right), self).export_diagnostics())
    ...
    def apply_resource_mode(self,name):
        ...
        self.left.set_resource_mode(mode);self.right.set_resource_mode(mode)
    ...
    def open_theme_gallery(self):
        ...
        self.select_page(2, open_dialog=False)

    def open_library(self):
        MediaLibraryDialog(self.settings, self).exec()
        try: self.store.save(self.settings)
        except Exception: pass

    def open_profiles(self):
        ProfileManagerDialog(self.settings,self.store,(self.left,self.right),self).exec()
        for card in (self.left,self.right):card.profile_box.blockSignals(True);card.profile_box.clear();card.profile_box.addItems(sorted(self.settings.profiles));card.profile_box.setCurrentText(self.settings.active_profile);card.profile_box.blockSignals(False)

    def open_settings(self):
        if SettingsDialog(self.settings,(self.left,self.right),self).exec():
    ...
    def save_all_profile(self,name):
        self.settings.profiles[name]={"0416:5408":self.left.profile(),"0416:5302":self.right.profile()};self.store.save(self.settings)
    ...
    def _setup_tray(self):
        ...
        self.tray_actions["Show / Open"].triggered.connect(self.restore_window);self.tray_actions["Hide"].triggered.connect(self.hide);self.tray_actions["Pause All"].triggered.connect(lambda:(self.left.pause(),self.right.pause()));self.tray_actions["Resume All"].triggered.connect(lambda:(self.left.play(),self.right.play()));self.tray_actions["Stop All"].triggered.connect(lambda:(self.left.stop(),self.right.stop()));self.tray_actions["Exit"].triggered.connect(self.exit_application)
    ...
    def shutdown(self):
        ...
        profiles={"0416:5408":self.left.profile(),"0416:5302":self.right.profile()};self.settings.profiles[self.settings.active_profile]=profiles
        ...
        if hasattr(self,"display_splitter"):
            sizes=self.display_splitter.sizes()
            if len(sizes)==2:self.settings.splitter_sizes=sizes
        self.settings.preview_scales={card.device_id:card.preview_scale for card in (self.left,self.right)};self.settings.advanced_expanded={card.device_id:card.advanced_toggle.isChecked() for card in (self.left,self.right)}
        self.monitor_timer.stop();self.reconnect_timer.stop();self.monitor_layouts_active.clear();self._shutdown_sensor_theme_runtime();self._stop_shared_decode();self.left.shutdown();self.right.shutdown();self.store.save(self.settings);self.tray.hide();self.tray.setContextMenu(None)
    ...
    def set_unified_view(self,enabled):
        enabled=bool(enabled and self.sync_toggle.isChecked());self.settings.unified_sync_view=enabled;self.display_splitter.setVisible(not enabled);self.unified_panel.setVisible(enabled)
    ...
    def update_unified_view(self):
        if not self.unified_panel.isVisible():return
        pixmap=self.left.preview.pixmap()
        ...
        self.unified_metrics.setText(f"9.16: {self.left.actual_fps():.1f} FPS · 6.86: {self.right.actual_fps():.1f} FPS")
    ...
    def _sync_brightness(self,source,value):
        ...
        if target.brightness_slider.value()!=value:target.brightness_slider.setValue(value)

    def _prepare_shared_decode(self,epoch):
        cards=(self.left,self.right)
    ...
    def _stop_shared_decode(self):
        ...
        if hub:hub.stop()
        if hasattr(self,"left"):self.left.shared_hub=None;self.right.shared_hub=None
    ...
    def _start_sensor_theme_output(self,device_id,theme,asset_root,fps,persisted_theme_id,save=True):
        ...
        if save:self.store.save(self.settings)
        self._update_sensor_theme_timer();self.render_sensor_theme_outputs()
    ...
    def render_sensor_theme_outputs(self):
        ...
        for device_id in tuple(runtime.active_device_ids):
            ...
            try:
                ...
                if card.window_visible and prepared.canvas is not None:card._show_image(card._qimage(prepared.canvas))
    ...
    def set_home_edit_mode(self,card,enabled,layout_override=None):
        ...
        layout=layout_override or (MonitorLayout.from_dict(raw) if raw else deepcopy(templates(card.device_id)[card.monitor_template.currentText()]))
    ...
    def render_monitor_layouts(self):
        ...
        for device_id,(layout,renderer,lease,mode) in tuple(self.monitor_layouts_active.items()):
            ...
            if card.window_visible:card._show_image(card._qimage(prepared.canvas));prepared.canvas.close()
    ...
    def _restore_safe_geometry(self,geometry):
        candidate=QRectF(*geometry).toRect();screens=QApplication.screens();screen=next((s for s in screens if s.availableGeometry().intersects(candidate)),None)
        if not screen:self._center_default_window();return
        area=screen.availableGeometry();candidate.moveLeft(max(area.left(),min(candidate.left(),area.right()-min(candidate.width(),area.width())+1)));candidate.moveTop(max(area.top(),min(candidate.top(),area.bottom()-min(candidate.height(),area.height())+1)));self.setGeometry(candidate)
    ...
    def retry_conflicts(self):
        ...
        if not remaining:
            self.left.reconnect();self.right.reconnect()
    ...
    def _apply_window_visibility(self):
        visible=self.isVisible() and not self.isMinimized();self.left.set_window_visible(visible);self.right.set_window_visible(visible);self._update_preview_visibility()
    ...
    def runtime_diagnostics(self):
        ...
        cards=[self.left.runtime_diagnostics(),self.right.runtime_diagnostics()];shared=getattr(self,"shared_hub",None);shared_active=bool(shared and shared.scheduler.is_active)
        return {"pid":os.getpid(),"total_oni_cpu_percent_normalized":total_cpu,"total_oni_ram_mb":rss,"total_oni_gpu_percent":"unavailable without Windows engine counters","oni_process_count":children+1 if isinstance(children,int) else children,"process_tree_thread_count":os_threads,"child_process_count":children,"processes":processes,"shared_decoder":shared_active,"shared_decoder_backend":getattr(shared.source,"backend","None") if shared else "None","scheduler_count":sum(x["scheduler_count"] for x in cards)+int(shared_active),"active_playback_workers":sum(x["playback_workers"] for x in cards)+(shared.active_workers if shared else 0),"active_timers":sum(1 for x in self.findChildren(QTimer) if x.isActive()),"active_decoders":sum(x["decoder_active"] for x in cards)+int(shared_active),"active_display_sessions":sum(x["display_session_active"] for x in cards),"decoded_queue_sizes":[x["decoded_queue_size"] for x in cards],"transport_queue_sizes":[x["transport_queue_size"] for x in cards],"gui_preview_presented_fps":[round(x.actual_preview_fps(),2) for x in (self.left,self.right)],"physical_lcd_completed_fps":[round(x.actual_fps(),2) for x in (self.left,self.right)],"retained_full_resolution_frames":sum(x["retained_full_resolution_frames"] for x in cards),"metrics_history_lengths":[x["metrics_history_length"] for x in cards],"cache_sizes":[x["cache_size"] for x in cards],"preview_pending":[x["preview_pending"] for x in cards]}
    ...
    def _update_preview_visibility(self):
        ...
        self.left.set_preview_enabled(enabled);self.right.set_preview_enabled(enabled)
    ...
    def _rebuild_display_splitter(self):
        ...
        self.display_splitter.setMinimumHeight(710 if stacked else 0);self.display_splitter.setMaximumHeight(16777215)
        for device_id in self.settings.display_order:
            self.display_splitter.addWidget(self.card_by_id[device_id])
        if self.settings.splitter_sizes and len(self.settings.splitter_sizes)==2:self.display_splitter.setSizes(self.settings.splitter_sizes)
        else:self.display_splitter.setSizes([1,1])
...
QWidget {
    ...
    font-size: 9pt;
    selection-background-color: #087db2;
...
QFrame#sidebar {
    background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                                stop:0 #060c16, stop:0.82 #081421, stop:1 #0a1b2b);
    border-right: 1px solid #174867;
}
