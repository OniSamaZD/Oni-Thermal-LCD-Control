...
class SensorThemeEditorPage(QWidget):
    ...
    def __init__(
        ...
        *, live_value_provider: Callable[[], object] | None = None,
        ...
        header = QHBoxLayout(); title = QLabel("ONI SENSOR STUDIO"); title.setObjectName("pageTitle"); header.addWidget(title)
        subtitle = QLabel("Compose sensor layouts at exact LCD pixels"); subtitle.setObjectName("pageSubtitle"); header.addWidget(subtitle); header.addStretch()
        ...
        background_button=QPushButton("Import Background");background_button.clicked.connect(self.import_background);toolbar.addWidget(background_button);self.actions["Import Background"]=background_button
        self.background_fit=QComboBox();self.background_fit.addItems(("cover","contain","stretch","center","native"));self.background_fit.setToolTip("Background placement")
        ...
        self.background_lock=QCheckBox("Lock BG");self.background_lock.setChecked(True)
        toolbar.addWidget(self.background_fit);toolbar.addWidget(self.background_opacity);toolbar.addWidget(self.background_lock)
        ...
        for label, operation in (("Align left", "left"), ("Align horizontal centers", "hcenter"), ("Align right", "right"), ("Align top", "top"), ("Align vertical centers", "vcenter"), ("Align bottom", "bottom"), ("Distribute horizontally", "distribute_h"), ("Distribute vertically", "distribute_v")):
        ...
        sensors_page=QWidget();sensors_layout=QVBoxLayout(sensors_page);sensors_layout.setContentsMargins(4,4,4,4)
        self.sensor_search=QLineEdit();self.sensor_search.setPlaceholderText("Search sensors")
        self.sensor_category=QComboBox();self.sensor_category.addItems(("All","CPU","GPU","Memory","Storage","Network","Cooling","Game/FPS"))
        self.sensor_list=QListWidget();self.sensor_bind_button=QPushButton("Bind to selected widget");self.sensor_refresh_button=QPushButton("Refresh availability");self._sensor_availability={}
        sensors_layout.addWidget(self.sensor_search);sensors_layout.addWidget(self.sensor_category);sensors_layout.addWidget(self.sensor_list,1);sensors_layout.addWidget(self.sensor_refresh_button);sensors_layout.addWidget(self.sensor_bind_button)

        self.left_tabs.clear();self.left_tabs.addTab(layers_page,"Layers");self.left_tabs.addTab(sensors_page,"Sensors");self.left_tabs.addTab(elements_page,"Widgets");self.left_tabs.addTab(themes_page,"Layouts")
        ...
        self.selection_status=QLabel("No selection");self.selection_status.setObjectName("muted")
        ...
        canvas_bar.addWidget(self.selection_status)
        ...
        self.sensor_search.textChanged.connect(self.refresh_sensor_browser);self.sensor_category.currentTextChanged.connect(self.refresh_sensor_browser);self.sensor_refresh_button.clicked.connect(self.refresh_sensor_availability);self.sensor_bind_button.clicked.connect(self.bind_selected_sensor);self.sensor_list.itemDoubleClicked.connect(lambda _item:self.bind_selected_sensor())
