...
ELEMENT_TYPES = frozenset({
    "text", "sensor_value", "sensor_label", "sensor_label_value", "value_unit", "image", "icon",
    "progress_bar", "horizontal_bar", "vertical_bar", "ring_gauge",
    "arc_gauge", "line_graph", "area_graph", "progress_indicator", "clock", "date", "fps", "frametime",
})
...
_TYPOGRAPHY_FIELDS = {
    ...
    "vertical_alignment", "letter_spacing", "text_color", "padding", "text_outline_color", "text_outline_width",
}
_SENSOR_FIELDS = {
...
def applicable_element_fields(element_type: str) -> frozenset[str]:
    ...
    if element_type in {"text", "sensor_value", "sensor_label", "sensor_label_value", "value_unit", "clock", "date", "fps", "frametime"}:
        allowed.update(_TYPOGRAPHY_FIELDS)
    if element_type in {"sensor_value", "sensor_label", "sensor_label_value", "value_unit", "fps", "frametime", "progress_bar", "progress_indicator", "horizontal_bar", "vertical_bar", "ring_gauge", "arc_gauge", "line_graph", "area_graph"}:
        allowed.update(_SENSOR_FIELDS)
    if element_type in {"progress_bar", "progress_indicator", "horizontal_bar", "vertical_bar", "ring_gauge", "arc_gauge"}:
        allowed.update(_GAUGE_BAR_FIELDS)
    if element_type in {"line_graph", "area_graph"}:
...
class ThemeElement:
    ...
    padding: float = 0.0
    text_outline_color: str = "transparent"
    ...
    def __post_init__(self) -> None:
        ...
        for key in ("x", "y", "width", "height", "rotation", "opacity", "font_size", "letter_spacing", "padding", "text_outline_width",
                    ...
                    "history_duration", "line_width", "scale_min", "scale_max", "animation_duration"):
        ...
        if self.padding < 0 or self.text_outline_width < 0:
            raise ThemeValidationError("padding and text outline width cannot be negative")
        ...
        if self.animation not in {"none", "smooth", "fade", "pulse"} or not 0.05 <= self.animation_duration <= 10:
            raise ThemeValidationError("invalid animation configuration")
        ...
        for key in ("text", "font_family", "font_file", "sensor_binding", "unit", "prefix", "suffix", "unavailable_text", "asset", "time_format", "date_format", "animation"):
        ...
        for key in ("text_color", "text_outline_color", "foreground_color", "background_color", "border_color", "shadow_color",
        ...
        if self.type in {"sensor_value", "sensor_label", "sensor_label_value", "value_unit", "fps", "frametime", "progress_bar", "progress_indicator", "horizontal_bar", "vertical_bar",
                         "ring_gauge", "arc_gauge", "line_graph", "area_graph"} and not self.sensor_binding:
...
class SensorTheme:
    ...
    def __post_init__(self) -> None:
        ...
        self.background_asset = _safe_member(str(self.background_asset), "assets/")
        if self.background_fit not in {"contain", "cover", "stretch", "center", "native"}:
            raise ThemeValidationError("background_fit must be contain, cover, stretch, center, or native")
        self.background_opacity = _finite(self.background_opacity, "background_opacity")
        if not 0 <= self.background_opacity <= 1:
            raise ThemeValidationError("background_opacity must be between 0 and 1")
        self.background_locked = bool(self.background_locked)
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "background_color": self.background_color, "background_asset": self.background_asset,
            "background_fit": self.background_fit, "background_opacity": self.background_opacity,
            "background_locked": self.background_locked, "schema_version": self.schema_version,
...
def validate_theme(theme: SensorTheme, available_resources: Iterable[str] = ()) -> None:
    ...
    if theme.background_asset and theme.background_asset not in available:
        raise ThemeValidationError(f"referenced theme resource is missing: {theme.background_asset}")
