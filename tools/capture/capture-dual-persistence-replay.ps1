...
$capture=Join-Path $root "captures\originals\live_dual_persistence_$stamp.pcapng"
$result=Join-Path $root "analysis\live-dual-persistence-result-$stamp.json"
...
try{
    ...
    if($cap.HasExited){throw "tshark/USBPcap exited before dual replay: $($cap.ExitCode)"}
    $env:PYTHONPATH=Join-Path $root 'src'
    'SEND BOTH CAPTURED FRAMES FOR 30 SECONDS' | & py -3.12 -m thermalright_lcd dual-persistence-live `
      --session-file (Join-Path $root 'analysis\session-report.json') `
      --pid5302-transaction (Join-Path $root 'analysis\pid5302-same-session-first-frame.json') `
      --pid5408-transaction (Join-Path $root 'analysis\pid5408-new-session-first-frame.json') `
